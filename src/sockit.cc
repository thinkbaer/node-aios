...
Sockit::Init(Local<Object> aExports) {
    ...
    // Add the constructor.
    // constructor.Reset(isolate, object->GetFunction(context));
    ...
    constructor.Reset();
    aExports->Set(String::NewFromUtf8(isolate, "Sockit"),
                  object->GetFunction(isolate->GetCurrentContext()).ToLocalChecked());
#endif
}
...
Sockit::Connect(const FunctionCallbackInfo<Value> &aArgs) {
    ...
    Local<Object> options = aArgs[0]->ToObject(isolate);
    ...
    if (!options->Has(context, hostKey).ToChecked() ||
        !options->Get(hostKey)->IsString()) {
        isolate->ThrowException(
                Exception::TypeError(
                        String::NewFromUtf8(
    ...
    if (!options->Has(context, portKey).ToChecked() ||
        !options->Get(portKey)->IsNumber()) {
        isolate->ThrowException(
                Exception::TypeError(
                        String::NewFromUtf8(
    ...
    String::Utf8Value optionsHost(isolate, options->Get(hostKey)->ToString(isolate));
...
Sockit::Read(const FunctionCallbackInfo<Value> &aArgs) {
    ...
    unsigned int bytesToRead = aArgs[0]->ToNumber(isolate)->Uint32Value(isolate->GetCurrentContext()).ToChecked();
...
Sockit::Write(const FunctionCallbackInfo<Value> &aArgs) {
    ...
    if (aArgs[0]->IsString()) {
        ...
        Local<String> dataString = aArgs[0]->ToString(isolate);
        String::Utf8Value data(isolate, dataString);
        // Write the data to the socket.
        success = sockit->Write(*data, dataString->Utf8Length(isolate));
    }
        //  else if(aArgs[0]->IsObject() &&
        //        aArgs[0]->ToObject()->GetConstructorName()->Equals(String::NewFromUtf8(isolate, "Buffer"))) {
    else if (aArgs[0]->IsObject()) {
        Local<Object> buffer = aArgs[0]->ToObject(isolate);
        success =
...
Sockit::SetPollTimeout(const FunctionCallbackInfo<Value> &aArgs) {
    ...
    Sockit *sockit = ObjectWrap::Unwrap<Sockit>(aArgs.Holder());
    sockit->mPollTimeout = aArgs[0]->Int32Value(isolate->GetCurrentContext()).ToChecked();
...
#else
    gDebugLog = aArgs[0]->BooleanValue(isolate);
#endif
    if (gDebugLog) {
