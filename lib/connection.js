...
// TODO load on demand!
    , net = require('net')
    , Sockit = require('sockit-to-me').Sockit
...
function Connection(options) {
    ...
    this.socketTimeout = options.socketTimeout || 10000;
            ...
            if (self.bytesRead > 0 && self.sizeOfMessage > 0) {
                ...
                if (self.stubBuffer != null && self.stubBuffer.length > 0) {
                    ...
                    if (data.length > 8) {
                        ...
                        if (sizeOfMessage > 8 && sizeOfMessage < self.maxBsonMessageSize && sizeOfMessage > data.length) {
                        ...
                        } else if (sizeOfMessage > 8 && sizeOfMessage < self.maxBsonMessageSize && sizeOfMessage == data.length) {
                            try {
                                var emitBuffer = data;
                                // Reset state of buffer
                                self.buffer = null;
                        ...
                        } else if (sizeOfMessage <= 8 || sizeOfMessage > self.maxBsonMessageSize) {
                            var errorObject = {
                                err: "socketHandler",
                                trace: null,
                                bin: data,
...
SyncSock.prototype.destroy = function () {
    if (this.connection) this.connection.close();
}


AsyncSock.prototype.destroy = function () {
...
SyncSock.prototype.write = function (buffer, skipread) {
    if (!Array.isArray(buffer)){
        this.connection.write(SyncSock.convertToBuffer(buffer));
    }else{
        for (var i = 0; i < buffer.length; i++) {
            this.connection.write(SyncSock.convertToBuffer(buffer[i]));
        }
    }
...
AsyncSock.prototype.write = function (buffer) {
...
AsyncSock.prototype.handleClose = function (hadError) {
    ...
    if (!hadError) {
        self.emit("close"
            , f("connection %s to %s:%s closed", conn.id, conn.host, conn.port)
            , self);
    }
}
...
AsyncSock.prototype.handleParseError = function (errorObject) {
    var self = this
    return self.emit("parseError", errorObject, self);
}
